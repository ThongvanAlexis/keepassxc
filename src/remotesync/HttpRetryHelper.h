...
{
    int maxRetries = 3;
    int baseDelayMs = 1000; // Exponential backoff with jitter
...
public:
     ...
     *
     * Backoff: up to base * 2^attempt, multiplied by random jitter in [0.5, 1.0)
     * (so with base=1000ms: 0.5-1s, 1-2s, 2-4s). Overridden by Retry-After when
     * present and larger.
     *
