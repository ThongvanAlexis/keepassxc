...
//   #1: 5-minute hard timeout fired.
//   #2: phishing-mitigation -- loginUrl host/port did not match configured server.
//   #3: initiate POST failure (network error / malformed JSON / missing
//       fields) -- same locked text in all three sub-cases.
//   #4: any polling-side hard failure (401/403/5xx/other-4xx, network error,
//       200+missing key, 200+empty value -- server defects surface through
//       the network-error banner).
// ---------------------------------------------------------------------------
static const char* const BANNER_1_TIMEOUT = "Nextcloud authorization timed out. Click Authorize to try again.";
static const char* const BANNER_2_PHISHING =
    "Nextcloud returned an unexpected authorization URL. Verify your server URL is correct.";
static const char* const BANNER_3_INITIATE_FAIL =
    "Could not start Nextcloud authorization. Verify your server URL and try again.";
static const char* const BANNER_4_NETWORK_ERROR =
...
void NextcloudLoginFlow::onInitiateFinished(QNetworkReply* reply)
{
    ...
    // Network error -> banner #3 (initiate failure). Same banner text
    // regardless of which sub-case fails (network, malformed JSON, missing
    // fields).
    if (reply->error() != QNetworkReply::NoError) {
        emitFailureWithBanner(tr(BANNER_3_INITIATE_FAIL));
        return;
    }
    ...
    if (!hostsAndPortsMatch(loginUrl, configuredUrl) || !hostsAndPortsMatch(pollEndpoint, configuredUrl)) {
        emitFailureWithBanner(tr(BANNER_2_PHISHING));
        return;
    }
...
void NextcloudLoginFlow::onPollFinished(QNetworkReply* reply)
{
    ...
    // hard failure: stop polling and emit banner #4. NOT a 4xx/5xx HTTP status
    ...
    if (reply->error() != QNetworkReply::NoError) {
        ...
        if (httpStatusOnError == 0) {
            stopPollingTimers();
            m_state = State::Failed;
            emit loginFailed(tr(BANNER_4_NETWORK_ERROR));
            return;
        }
    ...
    if (httpStatus == 200) {
        ...
        // the same banner #4 path as a network error -- the user sees
...
{
    // 5-minute hard cap reached. Stop everything and surface banner #1.
    stopPollingTimers();
    m_state = State::Failed;
    emit loginFailed(tr(BANNER_1_TIMEOUT));
}
