                                        ...
                                        QAtomicInt* abortFlag)
{
    ...
    for (int attempt = 0; attempt <= policy.maxRetries; ++attempt) {
        ...
        // Add jitter: multiply by random factor in [0.5, 1.0) to avoid thundering herd
        double jitter = 0.5 + QRandomGenerator::global()->generateDouble() * 0.5;
        int delayMs = static_cast<int>(qMin(static_cast<qint64>(baseDelay * jitter), static_cast<qint64>(INT_MAX)));
